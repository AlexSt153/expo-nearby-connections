...
- Native file transfer with `sendFile` on Android and iOS
- Incoming and outgoing progress events through `onFileTransferUpdate`
- Completed incoming file events through `onFileReceived`
- Active transfer cancellation through `cancelFileTransfer`
- Android `file://` and `content://` URI support
- Cache-backed received files with sanitized names and transfer isolation
